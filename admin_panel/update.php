...
<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $select_seller=$conn->prepare("SELECT * FROM `admin` WHERE id = ? LIMIT 1");
        $select_seller->execute([$admin_id]);
        ...
        if(!empty($email)){
            $select_email = $conn->prepare('SELECT * FROM `admin` WHERE id = ? AND email=?');
